defmodule Blitzy do
  def run(n_workers, url) when n_workers > 0 do
    ...
    |> Enum.map(&Task.await(&1, :infinity)) #equivalent to Enum.map(fn el -> Task.await(el) end)
  end

  defp parse_results(results) do
    {successes, _failures} = results |> Enum.partition(fn x -> case x do
        {:ok, _} -> true
        _ -> false
      end
    end)
